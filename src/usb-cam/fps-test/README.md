# USB Camera High-FPS Streamer & Frame Saver

> [!IMPORTANT]
> **Customized Version**: This project is modified from the original "Detect Objects on Camera" example. 
> It **disables AI object detection** to achieve the camera's maximum capture speed (High FPS) and adds a **continuous frame saving** feature to local storage via Web UI controls.

The **USB Camera High-FPS Streamer & Frame Saver** application lets you stream live feeds from a USB camera connected to the Arduino UNO Q at its maximum FPS, display the real-time resolution/FPS overlay, and save captured frames to local storage via Web UI buttons.

**Note:** This example must be run in **Network Mode** in the Arduino App Lab, since it requires a USB-C hub and a USB camera.

![USB Camera Stream](assets/docs_assets/video-object-detection.png)

This application continuously retrieves frames from a USB camera, streams them directly using FastAPI's `StreamingResponse` to achieve high-performance rendering (up to 30+ FPS), and provides interactive saving controls managed via an intuitive Web UI.
...
This project has been modified to prioritize high-speed video capture and frame saving over AI inference. Below are the key enhancements applied:

### 1. High-FPS Streaming (AI Inference Disabled)
* **Removed AI Inference Overhead**: The `video_object_detection` brick has been disabled in `app.yaml` to free up CPU and camera resources on the UNO Q's microprocessor (Cortex-A53).
* **Direct Camera Capture**: Utilized `arduino.app_peripherals.camera.Camera` directly in `python/main.py` to read frames using the low-level `_read_frame()` API.
* **FastAPI Streaming**: Implemented an MJPEG streaming endpoint (`/video_feed`) using FastAPI's `StreamingResponse` to push camera frames directly to the Web UI at the camera's maximum supported frame rate (typically up to 30 FPS depending on the USB camera).

### 2. UI Layout Simplification & Resolution Display
* **Dynamic Resolution & FPS overlay**: The Web UI now dynamically monitors and displays the active USB camera resolution (e.g., `640x360`) alongside the real-time capture FPS in the top-left overlay.
* **Streamlined UI**: Removed AI confidence controls, detection logs, and feedback panels to dedicate the interface entirely to a large, clean camera preview.

### 3. Continuous Frame Saving Feature
* **Start/Stop Capture Controls**: Integrated **"Start Saving"** and **"Stop Saving"** buttons on the Web UI.
* **Millisecond-Precision File Storage**: When saving is started, the Python backend writes every captured frame directly into the `output_images/` folder under the project directory. Files are named using millisecond-precision timestamps (e.g., `frame_YYYYMMDD_HHMMSS_ffffff.jpg`) to avoid name collisions and log sequence order.
