# PCB Component Detector (Customized version of Detect Objects on Camera)

> [!IMPORTANT]
> **Customized Version**: This project has been customized to run a custom Edge Impulse object detection model trained to detect **ICs (Integrated Circuits) and Connectors** on printed circuit boards.
> * **Dataset Used**: Trained using the [Roboflow 100 - Printed Circuit Board Dataset](https://universe.roboflow.com/roboflow-100/printed-circuit-board).
> * **Model Configuration**: Configured with the original custom model file: `camera-test-linux-aarch64-v4-impulse-#1.eim`.
> * **FPS Display**: The Web UI dynamically overlays and displays the real-time **Inference FPS** (Object Detection processing speed) at the top-left of the camera preview.

The **PCB Component Detector** application lets you detect ICs and Connectors on a live feed from a USB camera connected to the Arduino UNO Q and visualize bounding boxes around them in real-time.
...
This example hosts a Web UI where we can see the video input from the camera connected via USB. The video stream is then processed using the `video_objectdetection` Brick. When an object is detected, it is logged along with the confidence score (e.g. 95% IC).
