...
## Features
- **Event-Driven Image Capture & AI Inference**:
  - The camera stream is dynamically started (`video_stream_start`) and stopped (`video_stream_stop`) on sensor events to optimize power consumption and prevent camera module overheating.
  - The MCU stays in a deep sleep state (idle mode) until the external PIR sensor triggers a GPIO interrupt, achieving near-zero idle CPU usage.
- **Trigger Input**:
  - Connected to **D7** (MCU `PB2`) with Active-Low logic and internal Pull-up enabled (falling edge triggers the capture event).
- **Dual Visual Status & Classification Indicators**:
  - **LED 4 (Green)**: Toggles its state (ON ➡ OFF ➡ ON) every time the PIR sensor triggers, confirming detection events.
  - **LED 3 (RGB Color Classification)**: Turns ON for 3 seconds following a classification event to display results:
    - 🔵 **Blue LED** (PH12) turns on when `"pico"` is detected (controlled directly via GPIOH for hardware reliability).
    - 🟢 **Green LED** (`led3_green`) turns on when `"xiao"` is detected.
    - 🔴 **Red LED** (`led3_red`) turns on when `"nrf54l15"` is detected.
    - 🟡 **Yellow Color** (Red + Green combination) turns on when `"fpc"` is detected.
    - 📴 **All LEDs turn OFF** when returning to the idle waiting state.
...
### External PIR / Infrared Sensor Connections

| PIR Sensor Pin | Arduino Uno Q Pin | Connection Type / Target Port |
| :--- | :--- | :--- |
| **VCC** | **3.3V** (or 5V) | Power |
| **GND** | **GND** | Ground |
| **OUT** | **D7** | `PB2` (Interrupt Input, Active-Low with internal Pull-up) |
